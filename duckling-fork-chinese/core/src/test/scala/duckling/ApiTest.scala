...
import duckling.dimension.numeral.fraction.{Fraction, FractionData}
import org.scalatest.{FunSpec, Matchers}
import duckling.dimension.numeral.{DoubleSideIntervalValue, Numeral, NumeralValue, OpenIntervalValue}
      ...
      queries.foreach{
        query =>
          Api.analyze(query, testContext, options).foreach{
            answer =>
              answer.token.value match {
                case data: duckling.Types.NumeralValue  => println(data.schema)
                case _ => println("error")
              }
          }
      }
