  ...
  val ruleFewDuration = Rule(
    ...
    prod = tokens {
      case _ :: Token(TimeGrain, GrainData(g, _)) :: _ =>
        Token(Duration, DurationData(3, g, latent = true, schema = durationSchema(3, g)))
    }
  )
  ...
  val ruleCompositeDuration = Rule(
    ...
    prod = tokens {
      case t1 :: Token(TimeGrain, GrainData(g, _)) :: Token(_, dd@DurationData(_, dg, _, _)) :: _
        if g > dg && g != Month =>
        for (i <- getIntValue(t1)) yield Token(Duration, DurationData(i.toInt, g, schema = durationSchema(i.toInt, g)) + dd)
    }
  )
