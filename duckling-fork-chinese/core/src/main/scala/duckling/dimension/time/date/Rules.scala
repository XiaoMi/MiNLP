...
import duckling.dimension.matcher.Prods.{regexMatch, singleRegexMatch}
import duckling.dimension.numeral.Predicates.{getIntValue, isIntegerBetween}
import duckling.dimension.numeral.seq.{DigitSequence, DigitSequenceData}
import duckling.dimension.time.{GrainWrapper, TimeData}
import duckling.dimension.time.predicates._
import duckling.dimension.time.Prods.intersectDOM
import duckling.dimension.time.predicates.{EndOfGrainPredicate, SequencePredicate, TimeDatePredicate}
import duckling.dimension.time.duration.{isADecade, Duration, DurationData}
import duckling.dimension.time.enums.{Grain, Hint}
import duckling.dimension.time.enums.Grain._
import duckling.dimension.time.enums.Hint.{NoHint, RecentNominal, YearMonth}
import duckling.dimension.time.enums.IntervalType.{Closed, Open}
