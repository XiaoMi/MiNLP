...
import java.time.{LocalTime, Year => LYear}

import com.github.heqiao2010.lunar.{LunarCalendar, LunarData}

import duckling.Types.{conf, ZoneCN}
import duckling.dimension.time.Types.{TimeContext, TimeObject, _}
import duckling.dimension.time.enums._
import duckling.dimension.time.enums.AMPM._
    ...
    // 目前在农历与公历参考时间生成未来过去的序列上还有待梳理，在这里做事后补救
      ...
      future match {
        ...
        case ahead #:: nextAhead #:: _ =>
          val happened =
            td.timePred match {
              case a: TimeDatePredicate => timeBefore(ahead, refTime, td.timeGrain)
              case _ => false
            }
