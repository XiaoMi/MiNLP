...
import duckling.dimension.numeral.Prods._
import duckling.dimension.numeral.seq.{token => _, _}
...
trait Rules extends DimRules {
  ...
  val ruleIntegerWithThousandsSeparator = Rule(
    ...
    prod = singleRegexMatch {
      case text => parseDouble(text.replace(comma, "")).toOption.flatMap(double)
    }
  )

  val ruleDecimalNumeral =
    Rule(name = "digit <fraction>", pattern = List("\\.\\d+".regex), prod = singleRegexMatch {
      case m => parseDecimal(m)
    })
