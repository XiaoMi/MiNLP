...
object Prods {
  ...
  def parseDecimal(`match`: String): Option[Token] = {
    parseDouble(`match`).toOption.flatMap(double)
  }

  def token(nd: NumeralData) = Token(Numeral, nd)

  def double(x: Double): Option[Token] = Some(Token(Numeral, NumeralData(value = x)))
