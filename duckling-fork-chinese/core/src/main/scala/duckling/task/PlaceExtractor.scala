...
import duckling.Types.{Context, Options, ResolvedToken}

object PlaceExtractor {

  /**
    * 提取地点，只会返回一个结果
    * @param s 待提取串，先用langs的工具进行.normalize.noPunc比较好
    * @return
    */
  def extract(s: String): Option[String] = {
    val answers = Api.analyze(s, new Context(), Options(targets = Set(Place)))
